...
#include<log.h>
    ...
    {
    public:
		std::string RBobjectName;
		unsigned RBinstanceNumber;
		static unsigned RBinstanceCounter; 
		std::string RBobjectID;
    ...
    // protected:
		...
		/*Aggiungo un vettore che memorizza la penultima posizione del corpo (utile ai fini della collision detection)*/
    ...
    public:
		RigidBody(std::string objectName = "undefined")
		{
			++RBinstanceCounter;
			RBinstanceNumber = RBinstanceCounter;
			RBobjectName = objectName;
			RBobjectID = RBobjectName + "_" + std::to_string(RBinstanceNumber);
		}
       ...
       void _calculateTransformMatrix(Matrix4 &transformMatrix,
		...
		void setPrevPosition(const real x, const real y, const real z);
